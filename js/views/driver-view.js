...
export function renderDriverView(app) {
  ...
  const deliveryEnabled = app.store.state.platform.controls.deliveryEnabled;
...
function openDriverSupport(app, driver) {
  ...
  app.dialog.querySelector("#driverSupportForm").addEventListener("submit", event => {
    ...
    app.store.addSupportCase({ source: "driver", sourceId: driver.id, sourceName: driver.name, subject: app.dialog.querySelector("#driverSupportSubject").value, message: app.dialog.querySelector("#driverSupportMessage").value, priority: "normal" });
    app.closeDialog();
