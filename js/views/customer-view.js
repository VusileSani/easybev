...
function cartSummary(app, merchant, includeCheckout = true) {
  ...
  return `
    ...
    ${app.cart.length ? `
      ...
      ${includeCheckout ? (app.store.state.platform.controls.orderingEnabled ? `<button class="btn primary customer-primary-action" id="checkoutButton">Checkout · ${money(subtotal + delivery)}</button>` : `<div class="notice" style="margin-top:12px">New ordering is temporarily paused.</div>`) : ""}` : ""}`;
}
  ...
  return `
    ...
    <section class="customer-screen customer-menu-screen">
      ...
      <div class="customer-fulfilment-toggle" role="group" aria-label="Fulfilment method">
        ...
        ${selectedMerchant.delivery?.enabled && app.store.state.platform.controls.deliveryEnabled ? `<button class="${app.deliveryMode === "Home Delivery" ? "active" : ""}" data-delivery-mode="Home Delivery">Delivery · ${money(selectedMerchant.deliveryFee)}</button>` : ""}
      </div>
...
export function renderCustomerView(app) {
  ...
  if (selectedMerchant && (!selectedMerchant.delivery?.enabled || !app.store.state.platform.controls.deliveryEnabled) && app.deliveryMode === "Home Delivery") app.deliveryMode = "Takeaway";
...
function openCustomerSupport(app) {
  ...
  app.dialog.querySelector("#customerSupportForm").addEventListener("submit", event => {
    event.preventDefault();
    app.store.addSupportCase({ source: "customer", sourceId: customer.id, sourceName: customer.name, subject: app.dialog.querySelector("#customerSupportSubject").value, message: app.dialog.querySelector("#customerSupportMessage").value, priority: "normal" });
    app.closeDialog();
...
function openCheckout(app) {
  const controls = app.store.state.platform.controls || {};
  ...
  app.dialog.querySelector("#payButton")?.addEventListener("click", async event => {
    ...
    try {
      ...
      if (app.deliveryMode === "Home Delivery" && !app.store.state.platform.controls.deliveryEnabled) throw new Error("GoodKota delivery is temporarily unavailable.");
