...
export function renderAdminView(app) {
  ...
  const openCases = state.supportCases.filter(item => item.status !== "resolved");
  const settlementAttention = state.merchants.filter(merchant => !["verified"].includes(merchant.settlement?.status));
  const commercialAttention = state.merchants.filter(merchant => ["review", "overdue", "suspended"].includes(merchant.commercial?.status));
  const dispatchAttention = state.deliveryTasks.filter(task => task.status === "ready_for_dispatch" && !task.assignedDriverId);
  ...
  return `
    ...
    <section class="section">
      ...
      <div class="metric-strip">
        <div class="stat"><span class="muted">Merchants</span><b>${app.store.state.merchants.length}</b></div>
        <div class="stat"><span class="muted">Active</span><b>${app.store.state.merchants.filter(m => m.enabled && m.commercial?.status !== "suspended").length}</b></div>
...
function merchantsSection(app, qualityAlerts, commercialAttention) {
  const enabled = app.store.state.platform.controls.merchantOnboardingEnabled;
  return `
    ...
    <section class="section"><div class="table-wrap">${merchantTable(app.store.state.merchants)}</div></section>
...
function supportSection(app, openCases) {
  const cases = app.store.state.supportCases;
  return `
...
function communicationsSection(app) {
  return `
    ...
    <section class="section"><div class="table-wrap">${announcementTable(app.store.state.announcements)}</div></section>`;
}

function activitySection(app) {
  const events = app.store.state.auditTrail.filter(item => item.visibility !== "owner").slice(0, 80);
  return `
...
function bindCommonAdminActions(app, actor) {
  ...
  app.root.querySelectorAll("[data-quality-action]").forEach(button => button.addEventListener("click", () => {
    requestReason(app, {
      ...
      onConfirm: reason => {
        const merchant = app.store.merchant(button.dataset.merchantId);
        app.store.setQualityWorkflow(merchant.id, button.dataset.qualityAction, reason);
        app.store.logAudit({ actor, action: "quality_workflow_changed", targetType: "merchant", targetId: merchant.id, reason, visibility: "operations", metadata: { status: button.dataset.qualityAction } });
  ...
  app.root.querySelectorAll("[data-toggle-announcement]").forEach(button => button.addEventListener("click", () => {
    app.store.setAnnouncementActive(button.dataset.toggleAnnouncement, button.dataset.nextActive === "true", actor);
...
function supportSource(app, item) {
  if (item.merchantId) return app.store.merchant(item.merchantId)?.name || item.sourceName || "Merchant";
...
function openAddMerchant(app, actor) {
  ...
  app.dialog.querySelector("#merchantForm").addEventListener("submit", event => {
    ...
    const merchant = { id: uid("merchant"), name, legalName, contact: { email: app.dialog.querySelector("#merchantEmail").value.trim() }, area: app.dialog.querySelector("#merchantArea").value.trim(), address, latitude, longitude, prepMinutes: Number(app.dialog.querySelector("#merchantPrep").value) || 20, minOrder: Number(app.dialog.querySelector("#merchantMinimum").value) || 0, deliveryFee: Number(app.dialog.querySelector("#merchantDeliveryFee").value) || 0, delivery: { enabled: true, radiusKm: Number(app.dialog.querySelector("#merchantRadius").value) || 0, providerPreference: app.dialog.querySelector("#merchantProvider").value } };
    app.store.addMerchant(merchant);
    app.store.logAudit({ actor, action: "merchant_created", targetType: "merchant", targetId: merchant.id, reason: "Merchant onboarding", visibility: "operations" });
    app.closeDialog(); app.toast("Merchant created."); app.render();
  });
}

function openMerchantDetail(app, merchantId, actor) {
  const merchant = app.store.merchant(merchantId); if (!merchant) return;
  const compliance = complianceBadge(merchant.compliance?.status);
  const quality = qualityBadge(merchant.qualityWorkflow.status, merchant.qualitySummary.signal);
  ...
  app.dialog.querySelector("#toggleMerchantButton").addEventListener("click", () => requestReason(app, { title: merchant.enabled ? "Disable merchant" : "Enable merchant", description: "Record why GoodKota is changing this merchant's operating access.", confirmLabel: merchant.enabled ? "Disable" : "Enable", danger: merchant.enabled, onConfirm: reason => { const next = !merchant.enabled; app.store.setMerchantEnabled(merchantId, next); app.store.logAudit({ actor, action: next ? "merchant_enabled" : "merchant_disabled", targetType: "merchant", targetId: merchantId, reason, visibility: "operations" }); app.closeDialog(); app.toast(next ? "Merchant enabled." : "Merchant disabled."); app.render(); } }));
  app.dialog.querySelector("#saveComplianceButton").addEventListener("click", () => { const status = app.dialog.querySelector("#complianceStatus").value; const note = app.dialog.querySelector("#complianceNote").value; app.store.setMerchantCompliance(merchantId, status, note); app.store.logAudit({ actor, action: "merchant_compliance_changed", targetType: "merchant", targetId: merchantId, reason: note || status, visibility: "operations", metadata: { status } }); app.toast("Compliance updated."); openMerchantDetail(app, merchantId, actor); app.render(); });
  app.dialog.querySelector("#saveCommercialButton").addEventListener("click", () => { const status = app.dialog.querySelector("#commercialStatus").value; const plan = app.dialog.querySelector("#commercialPlan").value.trim(); const note = app.dialog.querySelector("#commercialNote").value; app.store.setMerchantCommercial(merchantId, { plan, status, note }, actor, note || `Commercial status ${status}`); app.toast("Commercial status updated."); openMerchantDetail(app, merchantId, actor); app.render(); });
}
  ...
  app.openDialog(`
    <div class="dialog-inner dialog-wide-inner"><div class="dialog-head"><div><span class="eyebrow">GoodKota Admin</span><h2>Edit merchant</h2></div><button class="icon-btn" data-close-dialog>✕</button></div>
      <form id="editMerchantForm" class="form-grid">
        ...
        <label class="field">Prep time (min)<input id="editMerchantPrep" type="number" min="1" value="${Number(merchant.prepMinutes || 20)}" /></label><label class="field">Minimum order (R)<input id="editMerchantMinimum" type="number" min="0" step="1" value="${Number(merchant.minOrder || 0)}" /></label><label class="field">Delivery fee (R)<input id="editMerchantDeliveryFee" type="number" min="0" step="1" value="${Number(merchant.deliveryFee || 0)}" /></label><label class="field">Delivery radius (km)<input id="editMerchantRadius" type="number" min="0" step="1" value="${Number(merchant.delivery?.radiusKm || 0)}" /></label><label class="field">Delivery provider<select id="editMerchantProvider">${providerOptions(merchant.delivery?.providerPreference)}</select></label><button class="btn primary field full">Save changes</button>
      </form></div>`);
  app.dialog.querySelector("#editMerchantResolveLocation").addEventListener("click", () => populateGeocode(app, "editMerchant"));
  app.dialog.querySelector("#editMerchantForm").addEventListener("submit", event => { event.preventDefault(); const latitude = Number(app.dialog.querySelector("#editMerchantLatitude").value); const longitude = Number(app.dialog.querySelector("#editMerchantLongitude").value); if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) return alert("Resolve the address or enter valid coordinates."); const changes = { name: app.dialog.querySelector("#editMerchantName").value.trim(), legalName: app.dialog.querySelector("#editMerchantLegalName").value.trim(), contact: { email: app.dialog.querySelector("#editMerchantEmail").value.trim() }, area: app.dialog.querySelector("#editMerchantArea").value.trim(), address: app.dialog.querySelector("#editMerchantAddress").value.trim(), latitude, longitude, prepMinutes: Number(app.dialog.querySelector("#editMerchantPrep").value) || 20, minOrder: Number(app.dialog.querySelector("#editMerchantMinimum").value) || 0, deliveryFee: Number(app.dialog.querySelector("#editMerchantDeliveryFee").value) || 0, delivery: { enabled: true, radiusKm: Number(app.dialog.querySelector("#editMerchantRadius").value) || 0, providerPreference: app.dialog.querySelector("#editMerchantProvider").value } }; app.store.updateMerchant(merchantId, changes); app.store.logAudit({ actor, action: "merchant_details_updated", targetType: "merchant", targetId: merchantId, reason: "Merchant profile maintenance", visibility: "operations" }); app.toast("Merchant details updated."); openMerchantDetail(app, merchantId, actor); app.render(); });
}

function openSupportCase(app, caseId, actor) {
  const item = app.store.state.supportCases.find(entry => entry.id === caseId); if (!item) return;
  app.openDialog(`<div class="dialog-inner"><div class="dialog-head"><div><span class="eyebrow">Support case</span><h2>${escapeHtml(item.subject)}</h2></div><button class="icon-btn" data-close-dialog>✕</button></div><div class="notice info">${escapeHtml(item.message)}</div><div class="form-grid" style="margin-top:14px"><label class="field">Status<select id="caseStatus"><option value="open" ${item.status === "open" ? "selected" : ""}>Open</option><option value="in_progress" ${item.status === "in_progress" ? "selected" : ""}>In progress</option><option value="resolved" ${item.status === "resolved" ? "selected" : ""}>Resolved</option></select></label><label class="field">Assigned to<select id="caseAssignee"><option value="">Unassigned</option>${app.store.state.platformStaff.filter(p => p.active !== false && ["admin", "owner"].includes(p.role)).map(p => `<option value="${p.id}" ${item.assignedTo === p.id ? "selected" : ""}>${escapeHtml(p.name)}</option>`).join("")}</select></label><label class="field full">Resolution / handover note<textarea id="caseNote" rows="4">${escapeHtml(item.resolutionNote || "")}</textarea></label><button class="btn primary field full" id="saveCaseButton">Save case</button></div></div>`);
  app.dialog.querySelector("#saveCaseButton").addEventListener("click", () => { app.store.updateSupportCase(caseId, { status: app.dialog.querySelector("#caseStatus").value, assignedTo: app.dialog.querySelector("#caseAssignee").value || null, resolutionNote: app.dialog.querySelector("#caseNote").value }, actor); app.closeDialog(); app.toast("Support case updated."); app.render(); });
}
...
function openAnnouncementDialog(app, actor) {
  ...
  app.dialog.querySelector("#announcementForm").addEventListener("submit", event => { event.preventDefault(); app.store.publishAnnouncement({ title: app.dialog.querySelector("#announcementTitle").value, message: app.dialog.querySelector("#announcementMessage").value, audience: app.dialog.querySelector("#announcementAudience").value, severity: app.dialog.querySelector("#announcementSeverity").value }, actor); app.closeDialog(); app.toast("Announcement published."); app.render(); });
}
